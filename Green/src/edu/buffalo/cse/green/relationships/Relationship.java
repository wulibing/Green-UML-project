	...
	public boolean isGeneric() {
		for (ASTNode feature : _features) {
			if (feature.getNodeType() == EXPRESSION_STATEMENT) {
				...
				return (node.getExpression().resolveTypeBinding().isParameterizedType());
			} else if (feature.getNodeType()
					== VARIABLE_DECLARATION_STATEMENT) {
				VariableDeclarationStatement node =
					(VariableDeclarationStatement) feature;
				
				return (node.getType().isParameterizedType());
			}
		}
